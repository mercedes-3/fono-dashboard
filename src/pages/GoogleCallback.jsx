  ...
  useEffect(() => {
    async function handleCallback() {
      ...
      try {
        ...
        const res = await fetch('https://nosnibbbggmlzavfylcd.supabase.co/functions/v1/google-oauth', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${session.access_token}`
          },
          body: JSON.stringify({ code, tenant_id: tenantId, redirect_uri: window.location.origin + '/google-callback' })
        })
