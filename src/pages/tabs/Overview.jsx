...
function Stat({ label, value, sub, color }) {
  return (
    <div className="card" style={{ position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ fontSize: 40, fontWeight: 300, fontFamily: 'var(--font-display)', color: color || 'var(--text)', letterSpacing: '-0.02em', lineHeight: 1 }}>{value}</div>
      {sub && <div style={{ fontSize: 10, color: 'var(--text-4)', marginTop: 10, letterSpacing: '0.03em' }}>{sub}</div>}
  ...
  return (
    <div style={{ padding: 40, maxWidth: 920 }}>
      <div style={{ marginBottom: 36 }}>
        <div style={{ fontSize: 10, color: 'var(--text-4)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 6 }}>{greeting}</div>
        <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 36, fontWeight: 300, color: 'var(--text)', letterSpacing: '-0.01em' }}>{tenant.name}</h1>
        {!tenant.twilio_number && (
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginTop: 14, padding: '8px 14px', background: 'var(--amber-dim)', border: '1px solid rgba(251,191,36,0.15)', borderRadius: 8, fontSize: 11, color: 'var(--amber)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontSize: 11, fontWeight: 500, color: 'var(--text)', letterSpacing: '0.02em' }}>Recent Leads</span>
          ...
          : leads.map(l => (
            <div key={l.id} style={{ padding: '12px 20px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
              <div style={{ minWidth: 0 }}>
                <div style={{ fontSize: 12, fontWeight: 500, color: 'var(--text)', marginBottom: 2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{l.full_name || l.phone || 'Unknown'}</div>
                <div style={{ fontSize: 10, color: 'var(--text-4)' }}>{l.service_type || 'No service'} · {l.phone?.startsWith('web_') ? 'Chat' : 'SMS'}</div>
              </div>
        ...
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontSize: 11, fontWeight: 500, color: 'var(--text)', letterSpacing: '0.02em' }}>Pending Appointments</span>
